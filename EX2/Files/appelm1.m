...
%    The second wave form has an amplitude of 3.

p2 = sin(1:20)*3;   % If input is wave with amplitude of 3
t2 = ones(1,20)*3;  % then output should be 3
...
net = newelm(p,t,[20],{'tansig','purelin'},'traingdx');
...
tic
[net,tr] = train(net,con2seq(p),con2seq(t));
toc
...
time = 1:length(p);
figure
plot(time,t,'--',time,cat(2,cell2mat(a)))
...
%a = sim(net,pgseq);
a = sim(net,con2seq(pg));

%    The network outputs and targets are plotted.

time = 1:length(pg);
figure
plot(time,tg,'--',time,cat(2,cell2mat(a)))
